...
import comando_submit_comprar_produto_IMP
...
def processa(ses, dados):
    ...
    e a quantidade desejada {qt} deve estar em {dados['quantidade']} (na forma de um string).
    
    A função acrescenta a quantidade {qt} do produto {prod} ao carrinho de compras da sessao
    {ses} dada, e devolve uma página com a descrição completa do produto, mostrando a 
    quantidade comprada."""
    return comando_submit_comprar_produto_IMP.processa(ses, dados)
