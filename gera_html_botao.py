...
def botao_submit(texto, cor_fundo):
  ...
  return gera_html_botao_IMP.submit_botao_simples(texto, cor_fundo)

def submit_compra():
  """Retorna um botão simples que é um fragmento HTML com o texto 'COMPRAR'."""
  return gera_html_botao_IMP.submit_comprar()

def submit_busca():
  """Retorna HTML do botão <submit> para uso em formulario de busca."""
  return gera_html_botao_IMP.submit_busca()
...
def submit_cadastrar():
  ...
  return gera_html_botao_IMP.submit_cadastrar()

def submit_login():
  """Retorna o botão para a submissão de login"""
  return gera_html_botao_IMP.submit_login()

def submit_logout():
  """Retorna o botão para a confirmação de logout"""
  return gera_html_botao_IMP.submit_logout()
